...
import { ALL_PRODUCTS } from "./ProductGrid";
...
export function FeaturedCategories() {
  const cats = [
    { label: "Earrings",     Icon: IconEarrings,   count: "240+ styles" },
    { label: "Necklaces",    Icon: IconNecklace,   count: "180+ styles" },
    { label: "Rings",        Icon: IconRing,        count: "120+ styles" },
    { label: "Bracelets",    Icon: IconBracelet,   count: "90+ styles"  },
    { label: "Bangles",      Icon: IconBangle,     count: "150+ styles" },
    { label: "Jewelry Sets", Icon: IconJewelrySet, count: "60+ styles"  },
  ];
...
/* ────────── BESTSELLERS ────────── */
export function BestSellers({ onAddToCart, onQuickView }: SectionsProps) {
  const products = ALL_PRODUCTS.filter((p) => p.isTrending).slice(0, 4);
  return (
    <section id="bestsellers" className="py-20 md:py-24" style={{ background: "#060400" }}>
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-end justify-between mb-12">
          ...
          <a href="#collections" className="hidden md:flex items-center gap-2 transition-opacity hover:opacity-70" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.85rem", color: "var(--rose-gold)", fontWeight: 500, textDecoration: "none" }}>
            View All <ArrowRight size={16} />
          </a>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {products.map((p) => <ProductCard key={p.id} product={p} onAddToCart={onAddToCart} onQuickView={onQuickView} />)}
        </div>
      </div>
    </section>
  );
...
/* ────────── NEW ARRIVALS ────────── */
export function NewArrivals({ onAddToCart, onQuickView }: SectionsProps) {
  const products = ALL_PRODUCTS.filter((p) => p.isNew).slice(0, 4);
  return (
    ...
    <section id="sale" className="py-16 md:py-20" style={{ background: "linear-gradient(135deg, #13100A 0%, #1A1500 50%, #13100A 100%)", borderTop: "1px solid rgba(201,168,76,0.2)", borderBottom: "1px solid rgba(201,168,76,0.2)" }}>
      <div className="max-w-4xl mx-auto px-4 md:px-8 text-center">
        ...
        <h2 className="mt-4 mb-3" style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(2rem, 5vw, 3.5rem)", fontWeight: 600, color: "#fff", lineHeight: 1.1 }}>
          Up to 40% Off<br /><em style={{ color: "#C9A84C" }}>On Bridal Collections</em>
        </h2>
        <p className="mb-8" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "1rem", color: "rgba(255,255,255,0.6)", fontWeight: 300 }}>
          Use code <strong style={{ color: "#E0C87A" }}>BRIDE40</strong> at checkout. Limited period offer only.
        </p>
        <div className="flex items-center justify-center gap-4 mb-8">
          {[{ val: "11", label: "Hours" }, { val: "47", label: "Mins" }, { val: "32", label: "Secs" }].map((t, i) => (
...
export function WhyChooseUs() {
  const features = [
    ...
    { icon: Truck, title: "Free Shipping", desc: "Complimentary delivery on all orders above ₹999 across India." },
...
/* ────────── TESTIMONIALS ────────── */
const reviews = [
  { name: "Priya Sharma", rating: 5, text: "Absolutely stunning quality! The rose gold earrings I ordered looked even better in person. Will definitely order again!", avatar: "PS", product: "Pearl Drop Earrings" },
  { name: "Ananya Rao", rating: 5, text: "Rosella has the most beautiful collection. Got my bridal set from here and received so many compliments. 10/10!", avatar: "AR", product: "Kundan Bridal Set" },
  { name: "Meera Patel", rating: 5, text: "Fast delivery, gorgeous packaging, and the jewelry quality is truly premium. My go-to for artificial jewelry.", avatar: "MP", product: "Floral Meenakari Set" },
  { name: "Ritu Agarwal", rating: 5, text: "The necklace I bought is so delicate and elegant. Couldn't believe it's artificial jewelry — looks completely real!", avatar: "RA", product: "Pearl Layered Necklace" },
];
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-auto pt-3 border-t" style={{ borderColor: "rgba(201,168,76,0.12)" }}>
                ...
                <div>
                  ...
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.7rem", color: "var(--rose-gold)" }}>{r.product}</p>
                </div>
...
export function InstagramGallery() {
  return (
    <section className="py-20 md:py-24" style={{ background: "#060400" }}>
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="text-center mb-12">
          <p style={eyebrow}>✦ @rosellalujoyjewels ✦</p>
